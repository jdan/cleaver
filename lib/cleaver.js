...
Cleaver.prototype._renderAgendaSlide = function (slices) {
  var titleInstructions = [{ level: 0 }]; // { indent: true/false, unindent: true/false, title: "The Title", level: 1 }

  for (var i = 0; i < slices.length; i++) { // for each slice
    var lines = slices[i].split(/(\n|\r)+/); // get every line independently of others

    for (var j = 0; j < lines.length; j++) { // for each text line
      var matches = /^(#+)\s+(.+)$/.exec(lines[j]); // parse the title and its hierarchy in each line
      
      if (!matches || !(matches.length == 3)) continue; // this is not a title to work with

      var currentLevel = matches[1].length; // h1, h2, h3...
      var titleText = matches[2];

      var lastTitleHierarchy = titleInstructions[titleInstructions.length-1];

      // for when the different of levels between titles is more than 1 at a time
      while(lastTitleHierarchy.level > currentLevel + 1
        || lastTitleHierarchy.level < currentLevel - 1) {
        if (lastTitleHierarchy.level > currentLevel) { // previous title was up high, we need to unindent one
          titleInstructions.push({ unindent: true, level: lastTitleHierarchy.level - 1 });
        } else if (lastTitleHierarchy.level < currentLevel) { // previous title was up low, we need to indent one
          titleInstructions.push({ indent: true, level: lastTitleHierarchy.level + 1 });
        }

        lastTitleHierarchy = titleInstructions[titleInstructions.length-1];
      }

      if (lastTitleHierarchy.level == currentLevel + 1) {
        titleInstructions.push({ unindent: true, level: currentLevel, title: titleText });
      } else if (lastTitleHierarchy.level == currentLevel - 1) {
        titleInstructions.push({ indent: true, level: currentLevel, title: titleText });
      } else { // they should be equal
        titleInstructions.push({ level: currentLevel, title: titleText });
      }
    
    } // for each text line
  } // for each slice

  // close everything that was left open
  for (var lastLevel = titleInstructions[titleInstructions.length-1].level; lastLevel > 0; lastLevel--) {
    titleInstructions.push({ unindent: true, level: lastLevel - 1 });
  }

  var templateData = { instructions: titleInstructions };
  return mustache.render(this.templates.loaded.agenda, templateData);
}
